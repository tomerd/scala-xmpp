package org.simbit.xmpp
{
	package protocol.presence
	{
		...
		object Unsubscribed
		{
			...
			def apply(id:Option[String], to:JID, from:JID):Unsubscribed =
			{
			...
			def unapply(unsubscribed:Unsubscribed):Option[(Option[String], JID, JID, Option[Seq[Extension]])] = Some(unsubscribed.id, unsubscribed.to, unsubscribed.from, unsubscribed.extensions)
		}
