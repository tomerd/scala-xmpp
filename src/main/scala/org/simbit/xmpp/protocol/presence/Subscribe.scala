package org.simbit.xmpp
{
	package protocol.presence
	{
		...
		object Subscribe
		{
			...
			def apply(id:Option[String], to:JID, from:JID):Subscribe =
			{
			...
			def unapply(subscribe:Subscribe):Option[(Option[String], JID, JID, Option[Seq[Extension]])] = Some(subscribe.id, subscribe.to, subscribe.from, subscribe.extensions)
		}
