package org.simbit.xmpp
{
	package protocol.iq
	{
		...
		object Set
		{
			...
			def apply(id:Option[String], to:JID, from:JID, extension:Option[Extension]=None):Set = apply(build(id, to, from, extension)) 
			...
			def unapply(set:Set):Option[(Option[String], JID, JID, Option[Extension])] = Some(set.id, set.to, set.from, set.extension)
		}
