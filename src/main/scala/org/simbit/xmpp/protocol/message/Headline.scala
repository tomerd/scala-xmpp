package org.simbit.xmpp
{
	package protocol.message
	{
		...
		object Headline
		{
			...
			def apply(to:JID, from:JID, body:String):Headline = apply(to, from, None, Some(body))
			
			def apply(to:JID, from:JID, subject:String, body:String):Headline = apply(to, from, Some(subject), Some(body))
			
			def apply(to:JID, from:JID, subject:Option[String], body:Option[String]):Headline = apply(None, to, from, subject, body, None, None)
