package org.simbit.xmpp
{
	package protocol.message
	{
		...
		object GroupChat
		{
			...
			def apply(to:JID, from:JID, body:String):GroupChat = apply(to, from, None, Some(body))
			
			def apply(to:JID, from:JID, subject:String, body:String):GroupChat = apply(to, from, Some(subject), Some(body))
			
			def apply(to:JID, from:JID, subject:Option[String], body:Option[String]):GroupChat = apply(None, to, from, subject, body, None, None)
