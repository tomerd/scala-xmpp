package org.simbit.xmpp
{
	package protocol.message
	{
		...
		object Error
		{
			...
			def apply(message:Message, condition:ErrorCondition.Value, description:Option[String]=None):Error = apply(message.id, message.to, message.from, message.extensions, condition, description) 
